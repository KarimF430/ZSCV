...
export default function BrandForm() {
  ...
  const { data: brand, isLoading } = useQuery<Brand>({
    ...
    queryFn: async () => {
      const res = await apiRequest('GET', `/api/brands/${id}`);
  ...
  useEffect(() => {
    if (brand && isEditing && !isLoading) {
      const brandFaqs = (brand.faqs as { question: string; answer: string }[]) || [];
      setFormData({
        name: brand.name,
        ...
        logo: brand.logo || '',
        faqs: brandFaqs.map((faq, index) => ({ ...faq, id: index.toString() })),
      });
      if (brand.logo) {
